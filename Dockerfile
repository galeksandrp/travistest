...
WORKDIR /root
RUN wget https://downloads.openwrt.org/releases/25.12.5/targets/x86/64/openwrt-imagebuilder-25.12.5-x86-64.Linux-x86_64.tar.zst \
  && tar --zstd -x -f openwrt-imagebuilder-*.tar.zst \
  && rm -rf openwrt-imagebuilder-*.tar.zst
WORKDIR /root/openwrt-imagebuilder-25.12.5-x86-64.Linux-x86_64
...
ENV CUSTOM_PACKAGES="nano-full luci-proto-wireguard htop luci-app-ttyd luci-app-attendedsysupgrade luci-app-keepalived luci-theme-openwrt-2020 luci-app-filemanager luci-app-dockerman"
RUN make image \
